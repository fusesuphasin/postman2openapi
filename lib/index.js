  ...
  for (let [i, element] of items.entries()) {
    ...
    if (element != null) {
      ...
      if (valid) {
        ...
        const { description, paramsMeta, bodyMeta, responseMeta } = descriptionParse(rawDesc)
...
function parseBodyData(data, bodyMeta) {
  ...
  let pathdata = Object.keys(data);

  let pathdataSub = getAllKeyPaths(data)
  for (const k of pathdataSub) {
    pathdata.push(k)
  }
  let path = ""
...
function parseRespData(bodies, language, data) {
  ...
  const bodyMeta = data
  const newData = example.example.data

  let pathdata = Object.keys(newData);
  let pathdataSub = getAllKeyPaths(newData)
  for (const k of pathdataSub) {
    pathdata.push(k)
  }
  let path = ""
...
function nestedObject(bodyMeta, data, pathdata, path) {
  ...
  for (var val of allKeys) {
    ...
    var object2 = Object.keys(data[val]);
...
function extractBody(body) {
  ...
  if (body != undefined) {
    ...
    if (body.proterties != "" && body.proterties != undefined) {
      let splitProterties = body.properties.split(",")
      for (const s of splitProterties) {
...
function descriptionParse(description) {
  ...
  const splitDesc = description.split(/# params-postman-to-openapi/gi)
  if (splitDesc.length === 1) return { description }
  const splitDesc2 = splitDesc[1].split(/# body-postman-to-openapi/gi)
  const splitDesc3 = splitDesc2[1].split(/# response-postman-to-openapi/gi)
  return {
    description: splitDesc[0].trim(),
    paramsMeta: parseMdTable(splitDesc2[0]),
    bodyMeta: parseMdTable(splitDesc3[0]),
    responseMeta: parseMdTable(splitDesc3[1])
  }
}
  ...
  const parsedResponses = Object.entries(statusCodeMap)
    .reduce((parsed, [status, { description, header, bodies }]) => {
      parsed[status] = {
        ...
        ...parseContent(bodies, responseMeta)
      }
    ...
    .reduce((content, [language, bodies]) => {
      if (language === 'json') {
        content['application/json'] = parseRespData(bodies, 'json', responseMeta)/* {
          //...parseExamples(bodies, 'json', responseMeta )
          parseRespData()
          schema: {
            type: 'object',
            required: [],
            example: parseExamples(bodies, 'json', responseMeta ),
            properties: parseRespData(responseMeta)
          }
        } */
      } else {
        content['text/plain'] = {
